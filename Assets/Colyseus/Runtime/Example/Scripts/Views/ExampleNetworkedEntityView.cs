...
using UnityEngine;
    ...
    public class ExampleNetworkedEntityView : ColyseusNetworkedEntityView
    {
        ...
        public bool IsEntityOwner { get; private set; } //Se owner for o player1, talvez eu consiga determinar os players através dessa variável
        public bool IsMine { get; private set; }
        public bool HasInit { get; private set; }
		public string CreationId { get; private set; } //MOD
	...
	//Essa função altera a posição, a rotação e a escala do objeto para valores indesejados, por isso vou anulá-la com um override ou simplesmente com // anulando as linhas de código
        protected virtual void SetStateStartPos()
        {
		    ...
		    //Essa linha estava gerando um "goleiro anão", nem o override adiantou, por isso não estou mais usando
        ...
        virtual protected void UpdateViewFromState()
        {
			...
			//É aqui que as informações de posição do player2 são passadas para o player1 e vice-versa
		...
		if (proxyStates[0].timestamp > interpolationTime || gameObject.tag == "player1" || gameObject.tag == "player2" || gameObject.tag == "score" || gameObject.tag == "keeperOpponent")
		{
			...
			if (syncLocalPosition)
			{
				myTransform.localPosition = Vector3.Lerp(myTransform.localPosition, proxyStates[0].pos, Time.deltaTime * (positionLerpSpeed + deltaFactor));
				if(gameObject.tag == "player1" || gameObject.tag == "player2")
                {
					if (state.zVel > 0.5f)
						myTransform.localPosition = new Vector3(myTransform.localPosition.x, (proxyStates[0].pos.y * 0.55f + myTransform.localPosition.y * 0.55f + 0.96f), myTransform.localPosition.z);
					else
						myTransform.localPosition = new Vector3(myTransform.localPosition.x, proxyStates[0].pos.y + 0.96f, myTransform.localPosition.z);
				}
			}

			if(!IsMine && gameObject.tag == "score")
            {
				myTransform.localPosition = proxyStates[0].pos;
            }
